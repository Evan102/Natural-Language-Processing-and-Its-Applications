...
from dataset import IntentForTrainDataset
...
def main(args):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(PRETRAINED_MODEL_NAME, num_labels=len(label2id) ).cuda()
    ...
    # Dataset define
    encoding_train = IntentForTrainDataset(traindata , label2id, tokenizer = tokenizer)
    encoding_valid = IntentForTrainDataset(validdata , label2id, tokenizer = tokenizer)
    ...
    for epoch in epoch_pbar:
        ...
        # print('train_tota',train_total)
        
        for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            
            model.train()
            # input_data = input_data.cuda()
            labelid = batch.pop('Intentid')
            ...
            train_correct += (predict == labelid.flatten()).sum().item()
        ...
        for batch in tqdm(valid_dataloader):
            ...
            with torch.no_grad():
                ...
                val_correct += (prediction == labelid.flatten()).sum().item()
                # print(prediction)
                # print(labelid.flatten())
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Directory to the dataset.",
        default="./data/intent/intent2idx.json",
    )
